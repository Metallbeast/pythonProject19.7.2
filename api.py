...
class PetFriends:
    ...
    def post_add_pet_photo (self, auth_key: json, pet_photo: str, pet_id: str) -> json:
        """Метод изменяет фото первого питомца в списке и возвращает статус
    ...
    def add_new_pet_with_photo(self, auth_key: json, name: str, animal_type: str, age: str, pet_photo: str) -> json:
        """Метод отправляет (постит) на сервер данные о добавляемом питомце и возвращает статус
        запроса на сервер и результат в формате JSON с данными добавленного питомца без применения MultipartEncoder"""
        headers = {'auth_key': auth_key['key']}
